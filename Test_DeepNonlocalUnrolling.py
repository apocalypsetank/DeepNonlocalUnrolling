...
def Recon_block(xt, x0, layer_no):
    ...
    x_mul1 = tf.matmul(x_phi_permuted, x_g_reshaped)
    x_mul2 = tf.matmul(x_phi_reshaped, x_mul1)
    x_mul2_softmax = tf.scalar_mul(1/((block_size+channel-1) *block_size), x_mul2)
  
    z2_temp = tf.reshape(x_mul2_softmax, [tf.shape(xt)[0], tf.shape(xt)[1], tf.shape(xt)[2], tf.shape(xt)[3]])
    z2 = tf.nn.relu(z2_temp)   
